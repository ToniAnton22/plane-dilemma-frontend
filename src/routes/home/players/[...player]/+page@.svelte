...
<a
	href="/home/players"
	class="w-6 h-6 md:w-12 md:h-12 m-2 md:m-6 hover:pointer z-40 absolute bottom-0 md:top-0"
>
	<svg
		aria-hidden="true"
		fill="none"
...
<Book>
	<div class="flex flex-row flex-wrap p-2 h-1/3 text-black" slot="page-one">
		<div class="m-[3vw] flex flex-row flex-wrap w-full">
			...
			<div class="flex w-1/2 flex-col mx-auto justify-center items-center h-1/2">
				<h1 class="text-2xl border-2 border-l-0 border-r-0 border-black text-center">
					{data?.player?.name}
				</h1>
				<p class="text-md text-center">"Better get off my nuts cuz I dont got none"</p>
			</div>
		</div>
	...
	<div
		class="w-full md:w-1/2 card h-full justify-center mx-auto self-center z-20 border-t-stone-900 space-x-4
    "
		slot="page-two"
	>
		<TabGroup
				...
				{#if tabSet === 0}
					{#if data?.player?.description != undefined}
						...
						<div class="md:hidden flex justify-center items-center align-center">
							<div
								class="md:hidden {theme} w-[12.7rem] h-[12.7rem] rounded-full relative flex justify-center items-center align-center"
							>
								<img
									src={data?.player?.avatar}
									class="z-10 rounded-full w-48 h-48 avatar-animation"
									alt="Kalahari"
								/>
							</div>
					...
					{/if}
				{:else if tabSet === 1}
					{#if data?.player?.background != undefined}
